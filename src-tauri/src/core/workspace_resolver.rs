...
impl WorkspaceResolver {
    ...
    pub fn new(workspace: &Path) -> Result<Self, String> {
        ...
        let canonical_root = workspace.canonicalize().map_err(|e| {
            format!(
    ...
    pub fn normalize_workspace_relative_path(&self, input: &str) -> Result<String, String> {
    ...
    pub fn resolve_existing(&self, requested: &str) -> Result<PathBuf, String> {
        ...
        if !canon_target.starts_with(&self.canonical_root) {
            return Err(format!(
                "PATH_OUTSIDE_WORKSPACE (Symlink Escape): {}",
                requested
            ));
        }
        ...
        if let Some(parent) = target_path.parent() {
            if parent.exists() {
                ...
                if !canon_parent.starts_with(&self.canonical_root) {
                    return Err(format!(
                        "PARENT_OUTSIDE_WORKSPACE (Symlink Escape): {}",
                        requested
                    ));
                }
            } else {
                ...
                while let Some(p) = current.parent() {
                    if p.exists() {
                        ...
                        if !canon_p.starts_with(&self.canonical_root) {
                            return Err(format!(
                                "ANCESTOR_OUTSIDE_WORKSPACE (Symlink Escape): {}",
                                requested
                            ));
                        }
                        break;
...
mod tests {
    ...
    fn test_accept_absolute_inside_workspace() {
        ...
        let norm = resolver
            .normalize_workspace_relative_path(&inside_file.to_string_lossy())
            .unwrap();
        assert_eq!(norm, "src/lib.rs");

        let resolved = resolver
            .resolve_existing(&inside_file.to_string_lossy())
            .unwrap();
    ...
    fn test_nested_creation_path() {
        ...
        let nested = resolver
            .resolve_for_create("nested/deep/directory/test.rs")
            .unwrap();
        ...
        assert!(resolver
            .resolve_existing("nested/deep/directory/test.rs")
            .is_ok());
