...
pub async fn download_asset(query: &str, output_path: &str) -> Result<String, String> {
    ...
    let enhanced_query = format!(
        "{}, simple video game asset, 2d, plain white background",
        query
    );
    let url = format!(
        "https://image.pollinations.ai/prompt/{}?width=256&height=256&nologo=true",
        encode(&enhanced_query)
    );
    ...
    if response.status().is_success() {
        ...
        Ok(format!(
            "Asset successfully generated and saved to {:?}",
            output_path
        ))
    } else {
        Err(format!(
