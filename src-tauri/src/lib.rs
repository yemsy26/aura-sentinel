...
#[tauri::command]
fn get_pending_mission() -> Option<serde_json::Value> {
    core::mission_persist::find_pending_mission().map(|resume| serde_json::json!({
        "objective": resume.objective, "workspace": resume.workspace_path,
        "step": resume.step, "role": resume.role,
    }))
}

#[tauri::command]
fn dismiss_pending_mission(workspace_path: String) -> Result<(), String> {
    let mut journal = core::session_journal::load_journal(&workspace_path);
    journal.interrupted = false;
    journal.status = "DESCARTADO".into();
    journal.fsm_context = None;
    journal.fsm_role = None;
    core::session_journal::save_journal(&workspace_path, &journal)
}
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            if let Some(job_path) = std::env::args().skip_while(|arg| arg != "--diagnose-mission").nth(1) {
                diagnostics::start(&handle, &job_path).map_err(std::io::Error::other)?;
                return Ok(());
            }
